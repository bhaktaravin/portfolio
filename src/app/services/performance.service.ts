...
export interface PerformanceMetric {
  ...
  source: 'navigation' | 'paint' | 'layout-shift' | 'first-input' | 'largest-contentful-paint' | 'custom';
}
...
export class PerformanceService {
      ...
      setTimeout(() => {
        ...
        if (navigation) {
          const navTiming: NavigationTiming = {
            ...
            domProcessingTime: Math.round(navigation.domComplete - navigation.domLoading),
          ...
          this.analytics.trackPerformance('DOMContentLoaded', navTiming.domContentLoaded, 'ms');
          this.analytics.trackPerformance('LoadComplete', navTiming.loadComplete, 'ms');
        }
      }, 0);
    });
  }
      ...
      const checkMemory = () => {
        ...
        if (memory) {
          ...
          if (memoryMetric.value > 50) {
            this.analytics.trackPerformance('HighMemoryUsage', memoryMetric.value, 'MB');
          }
        }
      };
  ...
  // Update Web Vitals
  private updateWebVitals(key: keyof WebVitals, metric: PerformanceMetric): void {
